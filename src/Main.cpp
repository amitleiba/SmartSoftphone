...
public:
    MCall(Account &acc, int call_id = PJSUA_INVALID_ID)
    : Call(acc, call_id)
    {
    	wav_player = NULL;
        myAcc = (MAccount *)&acc;
...
public:
    ...
    {
        Call *call = new MCall(*this, iprm.callId);
...
void MCall::onCallTransferRequest(OnCallTransferRequestParam &prm)
{
    /* Create new Call for call transfer */
    prm.newCall = new MCall(*myAcc);
}

void MCall::onCallReplaceRequest(OnCallReplaceRequestParam &prm)
{
...
static void mainProg1(MyEndpoint &ep)
{
    ...
    acc_cfg.idUri = "sip:1003@192.168.127.93:9515";
    acc_cfg.regConfig.registrarUri = "sip:192.168.127.93:9515";

#if PJSIP_HAS_DIGEST_AKA_AUTH
    AuthCredInfo aci("Digest", "*", "test", PJSIP_CRED_DATA_EXT_AKA | PJSIP_CRED_DATA_PLAIN_PASSWD, "passwd");
    aci.akaK = "passwd";
#else
    AuthCredInfo aci("digest", "*", "1003", 0, "12345678");
#endif
    ...
    // Make outgoing call
    Call *call = new MCall(*acc);
    acc->calls.push_back(call);
    CallOpParam prm(true);
    prm.opt.audioCount = 1;
    prm.opt.videoCount = 0;
    call->makeCall("sip:1000@192.168.127.93:9515", prm);
...
static void mainProg4(MyEndpoint &ep)
{
    ...
    // Add account
    AccountConfig acc_cfg;
    acc_cfg.idUri = "1002@192.168.1.46";
    acc_cfg.regConfig.registrarUri = "192.168.1.46";
    MAccount *acc(new MAccount);
    acc->create(acc_cfg);
